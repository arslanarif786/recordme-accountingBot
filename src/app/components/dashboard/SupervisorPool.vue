<template>
  <div>
    <AppBar />
    <div v-if="userOnline">
      <SupervisorPage v-if="infoCheck === 'done'" />
      <Loader v-else-if="infoCheck === 'wait'">
        <template v-slot:text>Please wait ...</template>
      </Loader>
      <div
        class="d-flex flex-column justify-center align-center"
        v-else-if="infoCheck === 'show-popup' || infoCheck === 'leave-portal'"
      >
        <Loader>
          <template v-if="infoCheck === 'show-popup'" v-slot:text
            >You are already logged in elsewhere.</template
          >
          <template v-else-if="infoCheck === 'leave-portal'" v-slot:text
            >This account has updated its current login status</template
          >
        </Loader>
        <v-btn
          color="appColor"
...
import SupervisorPage from "../supervisor-portal";
