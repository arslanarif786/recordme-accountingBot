      ...
      <v-card>
        ...
        <v-container class="py-0 pt-1" v-if="!loader">
          <v-card-text class="py-0 font-style-paragraph">
            Note : Ticket will be moved to selected accountant
          </v-card-text>
          <v-col cols="12" class="ma-0 px-0">
            <v-select
              class="appColor--text default-input-field ma-1 pa-0"
              :items="[...accountants, { user: false, userId: {name : 'Anyone ... '} }]"
              flat
              solo
              hide-details
              placeholder="Status"
...
export default {
  name: "RerouteTicketSupervisorDialog",
  data() {
    return {
      loader: true,
      dialog: false,
  ...
  mounted() {
    ...
    EventBus.$on("open-reroute-ticket-supervisor-dialog", (object) => {
      this.dialog = true;
      this.ticket = object;
      //call api here and set loader to false
      this.getActiveAccountants();
    });
  },
  computed: {
    ...mapGetters({
  ...
  methods: {
    ...
    rerouteTicket() {
      ...
      if (this.selectedAccountant.trim().length == 0) {
      ...
      if (this.selectedAccountant !== "Anyone...") {
      accountant = this.accountants.find(
        (item) => item.userId.name === this.selectedAccountant
      );}
      ...
      axios
        .post(
          `${UrlServices.rerouteTicketToAccountant}/${this.ticket._id}?accountantId=${accountant?.userId?._id}`,
          data,
          {
            headers: {
    ...
    getActiveAccountants() {
      console.log("all accountants>>>");
      axios
        .get(
          `${UrlServices.getActiveAccountantsForSpecificWorkspace}?workspaceId=${this.ticket.actualWorkspaceId}`,
          {
            headers: {
              Authorization: this.token,
            },
        ...
        .then((res) => {
          console.log("users Accountant--->", res.data.users);
          this.accountants = res.data.users;
          this.loader = false;
        })
        .catch((e) => {
          EventBus.$emit("show-snackbar", {
